### 前置条件

- ✅ Git 仓库
- ✅ 代理软件（如果在中国大陆）
- ✅ Google AI Studio API Key


## 🚀 完整安装流程


### 步骤 1: 全局安装 GitAnalyzer 启动服务

```bash
# 在 GitAnalyzer 目录中运行
cd /path/to/GitAnalyzer
bash .git-scripts-install/git-analyzer-global-installer.sh

# 重新加载 shell 配置
source ~/.zshrc
```


### 步骤 2: 配置 Gemini API

**方式一：使用配置向导（推荐）**

```bash
# 运行配置向导
cd /path/to/GitAnalyzer
./setup_gemini_api.sh
```

**配置向导会：**
1. 检测已有配置（如有则自动使用）
2. 提示你输入 API Key（从 https://aistudio.google.com/app/apikey 获取）
3. 自动测试 API Key 是否有效
4. 保存配置到模板文件
5. 更新所有项目配置

**方式二：手动编辑配置文件**

直接编辑配置模板：`/path/to/GitAnalyzer/.git-scripts-logs/.git-analyzer-config.json`

```json
{
  "enabled": true,
  "output_base_dir": "code_summaries",
  "gemini_model": "gemini-2.0-flash-exp",
  "gemini_api_key": "",
  "http_proxy": "http://127.0.0.1:7897",
  "https_proxy": "http://127.0.0.1:7897",
  "log_file": ".git-analyzer.log",
  "max_diff_size": 50000,
  "timeout_seconds": 60,
  "notification": {
    "show_success": true,
    "show_errors": true
  }
}
```


### 步骤 3: 查看服务状态

```bash
git-analyzer-status
```

### 步骤 4: 列出已注册项目

```bash
git-analyzer-list
```

查看当前已经注册到 GitAnalyzer 的所有项目。

### 步骤 5: 启动服务

```bash
git-analyzer-start
```

启动 GitAnalyzer 全局服务，所有已注册项目的提交都将被自动分析。

**⚠️ 重要提示：**
- 如果首次启动时未配置 API Key，系统会提示你进行配置
- 配置完成后，需要重新运行 `git-analyzer-start` 重启服务
- 重启后会自动更新所有已注册项目的全局配置

### 步骤 6（可选）：停止服务

```bash
git-analyzer-stop
```

如需暂停分析，可以停止服务。

### 步骤 7: 注册项目

在你的项目目录中运行：

```bash
cd /path/to/your/project
register.sh
```

这会：
- 创建 Git post-commit 钩子
- 创建项目配置文件
- 在 GitAnalyzer 中创建项目日志目录
- 分析目录

### 步骤 8: 正常使用 Git

```bash
git add .
git commit -m "your commit message"
# 代码分析会自动在后台运行
```


## 📝 项目配置

配置文件位置：`项目根目录/.git-scripts-logs/.git-analyzer-config.json`

```json
{
  "enabled": true,   // 订阅的 Gemini API
  "output_base_dir": "code_summaries", // 代码摘要目录
  "gemini_model": "gemini-2.0-flash", // 使用的模型
  "gemini_api_key": "YOUR_API_KEY_HERE", // API Key
  "max_diff_size": 50000, // 最大差异大小
  "timeout_seconds": 120, // API 超时时间（秒）
  "http_proxy": "http://127.0.0.1:7897", // HTTP 代理地址
  "https_proxy": "http://127.0.0.1:7897", // HTTPS 代理地址
  "notification": {
    "show_success": true, // 显示成功通知
    "show_errors": true // 显示错误通知
  }
}
```

**重要配置项说明：**

| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| `enabled` | 使用开关 | `true` |
| `gemini_model` | 使用的模型 | `gemini-2.0-flash` |
| `gemini_api_key` | API Key（必填） | - |
| `http_proxy` | HTTP 代理地址 | `http://127.0.0.1:7897` |
| `https_proxy` | HTTPS 代理地址 | `http://127.0.0.1:7897` |
| `timeout_seconds` | API 超时时间（秒） | 120 |

**可用的模型：**
- `gemini-2.5-flash` - 最新最快
- `gemini-2.0-flash` - 推荐，速度快
- `gemini-2.5-pro` - 更强大，但较慢

---

### 自动分析

每次提交代码时，会自动触发分析：

```bash
git add .
git commit -m "你的提交信息"
# 🚀 代码分析已在后台启动...
```

### 查看分析结果

**查看日志：**
```bash
# 实时查看
tail -f ~/GitAnalyzer/项目名/analyzer.log

# 查看最近50行
tail -50 ~/GitAnalyzer/项目名/analyzer.log
```

**查看分析报告：**
```bash
# 分析报告保存在
ls ~/GitAnalyzer/项目名/code_summaries/

# 查看最新报告
ls -lt ~/GitAnalyzer/项目名/code_summaries/ | head -5
```

---

## 🔧 常用命令

### 服务管理

```bash
git-analyzer-start    # 启动服务
git-analyzer-stop     # 停止服务
git-analyzer-status   # 查看状态
git-analyzer-list     # 列出已注册项目
```

### 项目管理

```bash
# 注册项目
cd /path/to/project
register.sh

# 注销项目
cd /path/to/project
unregister.sh
```

---

## ⚠️ 常见问题

### 问题 1: API 调用失败

**症状**: 日志显示 API 调用失败

**解决方案**:
1. 检查 API Key 是否正确
2. 检查代理设置是否正确
3. 确认代理软件正在运行
4. 测试网络连接：
   ```bash
   export http_proxy="http://127.0.0.1:7897"
   export https_proxy="http://127.0.0.1:7897"
   curl -s "https://generativelanguage.googleapis.com/v1/models?key=YOUR_API_KEY"
   ```

### 问题 2: 分析报告未生成

**可能原因**:
- 服务未启动
- 项目配置中 `enabled` 为 false
- API Key 无效

**解决方案**:
```bash
# 1. 检查服务状态
git-analyzer-status

# 2. 检查项目配置
cat .git-scripts-logs/.git-analyzer-config.json

# 3. 查看日志
tail -50 ~/GitAnalyzer/项目名/analyzer.log
```

### 问题 3: 代理不工作

**症状**: 日志显示连接超时

**解决方案**:
1. 确认代理端口正确（默认 7897）
2. 确认代理软件正在运行
3. 测试代理：
   ```bash
   curl -x http://127.0.0.1:7897 https://www.google.com
   ```
4. 更新配置文件中的代理设置

### 问题 4: 模型不可用 (404 错误)

**症状**: API 返回 404，提示模型不存在

**解决方案**:
1. 查询可用模型：
   ```bash
   export http_proxy="http://127.0.0.1:7897"
   curl -s "https://generativelanguage.googleapis.com/v1/models?key=YOUR_API_KEY" | \
   jq -r '.models[] | select(.supportedGenerationMethods[] | contains("generateContent")) | .name'
   ```
2. 更新配置文件中的 `gemini_model` 为可用模型

---